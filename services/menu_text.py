"""Сборка текста главного меню: приветствие + карточка стиха дня + счётчики."""
import html
import urllib.parse
...
async def build_menu_text(user: User | None, lang: str, bot=None) -> str:
    """
    Текст главного меню: приветствие + подзаголовок + карточка стиха дня + счётчики.
...
def _build_progress_block(user: User, lang: str) -> list[str]:
    """Блок «Мой прогресс»: счётчик дней со Словом + счётчик дней в молитве.
