...
**Donations.** Telegram Stars flow lives in `handlers/donate.py` and `services/donate_service.py`. `PreCheckoutQuery` must always be answered with `ok=True` for Stars to clear; successful payments insert a `Donation` row and notify `ADMIN_IDS`. The donate screen shows all methods directly — no region-selection step. `donate_main_keyboard(lang, in_spain)` gates each button: **Monobank** for uk/ru UI users, **Bizum** for users in a Spanish timezone (`SPAIN_ZONES` = Madrid/Canary), **Revolut/PayPal/Crypto** for everyone. Every external-URL button is additionally gated on its env var being non-empty — see `keyboards/donate.py`. Donation links/card live only in `.env` (`DONATE_MONOBANK_URL` / `DONATE_MONOBANK_CARD` / `DONATE_REVOLUT_URL` / …); `config.py` defaults to empty so no secrets are committed.
